...
len(np.unique(np.concatenate([cptmems, phemems])))
...
Cases_LR['y'] = 1
Control_LR['y'] = 0
trainsize = int(round(Cases_LR.shape[0]*testratio))
train_data = pd.concat([Control_LR[:(int(ccmultiplier)*trainsize)], Cases_LR[:trainsize]])
test_data = pd.concat([Control_LR[(int(ccmultiplier)*trainsize):Control_LR.shape[0]], Cases_LR[trainsize:Cases_LR.shape[0]]])
...
lg = LogisticRegression(solver = 'lbfgs', verbose = True, max_iter=100)
